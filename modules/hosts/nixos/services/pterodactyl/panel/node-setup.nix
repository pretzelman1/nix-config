{
  ...
  generateUUID = name: "00000000-0000-0000-0000-${lib.substring 0 12 (builtins.hashString "sha256" name)}";
  ...
  mkAllocationSetup = name: node: let
    ...
    mkAllocationInsert = alloc: ''
      ...
      SELECT
        ...
        '${escapeSQL alloc.ip}',
        ${
        if alloc.ipAlias != null
        then "'${escapeSQL alloc.ipAlias}'"
        else "NULL"
      },
        ${toString alloc.port},
        ${
        if alloc.notes != null
        then "'${escapeSQL alloc.notes}'"
        else "NULL"
      },
        NOW(),
        NOW()
  ...
  in ''
    ...
    ${
      ...
      then ''
        ...
        EOF
      ''
      else "echo 'No allocations to create for node ${node.name}'"
    }
  '';
in
